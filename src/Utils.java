...
public class Utils {
    ...
    static void writeParameters(double c1, double c2, int size_pop, String name_function) {
        File file = new File(Main.path + name_function + "/Parametros c1 e c2 - Tamanho_enxame " + size_pop + ".txt");
        try {
            try (Writer bw = new BufferedWriter(new FileWriter(file, true))) {
    ...
    static void writeConvergence(double convergence, int size_pop, int generations, String name_functions) {
        File file = new File(Main.path + name_functions + "/Solucao cada geracao (convergencia) - " + size_pop + ".txt");
        try (Writer bw = new BufferedWriter(new FileWriter(file, true));) {
